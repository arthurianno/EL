...
class GetSalePointsInBoundsUseCase @Inject constructor(
    private val repository: SalePointsRepository,
    schedulers: SchedulersFacade
) : ObservableListUseCase<SalePoint, GetSalePointsInBoundsUseCase.Params>(schedulers) {

    override fun buildUseCaseObservable(params: Params?): Observable<List<SalePoint>> =
        repository.getSalePoints(checkNotNull(params).bounds)

    data class Params(val bounds: CoordinatesBounds)
}