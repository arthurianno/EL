...
class DateFormatExtensionsTest {
    ...
    fun simpleDateFormatterCache_SetAndGetFormatter_SameAndEquals() {
        ...
        val dateFormat = SimpleDateFormat(
            CommonFormats.FORMAT_STANDARD_DATE_FULL_MILLIS_UTC,
            Locale.getDefault()
        )
    ...
    fun getOrCreateFormatter_TheSameFormat_TheSameTimeZone_TheSameLocale_SameAndEqual() {
        val dateFormat1 = SimpleDateFormatterCache.getOrCreateFormatter(
            CommonFormats.FORMAT_SIMPLE_DATE,
            TimeZone.getTimeZone("UTC"),
            Locale.getDefault()
        )
        val dateFormat2 = SimpleDateFormatterCache.getOrCreateFormatter(
            CommonFormats.FORMAT_SIMPLE_DATE,
            TimeZone.getTimeZone("UTC"),
            Locale.getDefault()
        )
    ...
    fun getOrCreateFormatter_DifferentFormats_SameTimeZone_SameLocale_NotSameAndNotEqual() {
        val dateFormat1 = SimpleDateFormatterCache.getOrCreateFormatter(
            CommonFormats.FORMAT_SIMPLE_DATE_TIME,
            TimeZone.getDefault(),
            Locale.getDefault()
        )
        val dateFormat2 = SimpleDateFormatterCache.getOrCreateFormatter(
