...
import com.elta.android.domain.features.rostech.interactor.ConnectIomtUseCaseVariantA
...
import com.elta.android.presentation.features.sync.control.bluetoothControl2VariantA
...
class HomeFlowPmVariantA @Inject constructor(
    ...
    private val syncWithGlucometerUseCase: SyncWithGlucometerUseCaseVariantA,
    ...
    private val connectIomt: ConnectIomtUseCaseVariantA,
    ...
    val manualSyncError = state<ManualSyncErrorVariantA>()
    ...
    override fun onCreate() {
        ...
        selectedItemIdState.observable
            .doOnNext { id -> Log.i(NAV_TRACE_TAG, "HomeFlowPmVariantA.selectedItemIdState(id=$id)") }
            .subscribe()
            .untilDestroy()
    ...
    private fun observeClicks() {
        ...
        menuItemSelectedAction.observable
            .doOnNext { id ->
                Log.i(
                    NAV_TRACE_TAG,
                    "HomeFlowPmVariantA.menuItemSelectedAction(id=$id,current=${selectedItemIdState.valueOrNull})"
                )
            }
            ...
            .doOnNext { pair ->
                Log.i(
                    NAV_TRACE_TAG,
                    "HomeFlowPmVariantA.navigateToTab(screen=${pair.second.first.screenKey}, event=${pair.second.second})"
                )
            }
            ...
            .map { it.first }
            .doOnError { error ->
                Log.e(NAV_TRACE_TAG, "HomeFlowPmVariantA.menuItemSelectedAction stream error", error)
                handleError(error)
            }
            .retry()
            .subscribe(selectedItemIdState.consumer)
    ...
    private fun syncWithGlucometer(isAuto: Boolean): Observable<Int> =
        syncWithGlucometerUseCase.execute(SyncWithGlucometerUseCaseVariantA.Params())
            .concatWith(
    ...
    private fun observableSyncError(
        ...
        is LocationNotEnabledErrorVariantA -> locationEnableAndRepeat(isAuto)
    ...
    private fun handleManualSyncError(error: Throwable) {
        ...
        when (error) {
            is BluetoothNotEnabledErrorVariantA, LocationNotEnabledErrorVariantA -> {
                manualSyncError.accept(ManualSyncErrorVariantA.ErrorSync)
            }
            ...
            is GlucometerOfflineError -> {
                ...
                manualSyncError.accept(ManualSyncErrorVariantA.NotFound)
    ...
    private fun handleAutoSyncError(error: Throwable) {
        when {
            error is BluetoothNotEnabledErrorVariantA || error is LocationNotEnabledErrorVariantA ||
                    error.cause is BluetoothNotEnabledErrorVariantA || error.cause is LocationNotEnabledErrorVariantA ||
                    error is LocationPermissionNotGrantedErrorVariantA || error is CommandError || error is BluetoothScannerError ->
