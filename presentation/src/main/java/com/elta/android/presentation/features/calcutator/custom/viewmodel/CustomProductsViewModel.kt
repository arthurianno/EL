...
import com.elta.android.domain.features.calculator.interactor.SearchCustomProductUseCase
...
class CustomProductsViewModel @Inject constructor(
    ...
    private val searchCustomProducts: SearchCustomProductUseCase,
    private val deleteProductUseCase: DeleteProductUseCase,
) : BaseViewModel<CustomProductsViewState>() {
    override fun createInitState(): CustomProductsViewState = CustomProductsViewState(
        searchInFocus = false, calculatorFlow = CalculatorFlow.BREAD_UNITS, isError = false
    )

    val appTopBar = BaseAppTopBarWidgetModel()
    val searchField = SearchFieldWidgetModel()

    val confirmDialog = BaseDialogWidgetModel<DishUiEntity>(positiveOnCLick = { dish ->
        dish?.let {
            deleteProduct(dish)
        }
    })
    ...
    override val widgets: List<BaseWidgetModel<*>> = listOf(
        appTopBar, searchField
    ).actionObserve()

    init {
        launch {
            searchField.state.map { it.textField.text }.debounce(DEBOUNCE_MILLIS)
                .collect { searchText ->
    ...
    override fun handleUserAction(action: Action) {
        when (action) {
            ...
            is CustomProductAction.CreateProduct -> router.navigateTo(
                Screens.CreateCustomProductScreen(
                    calculatorFlow = state.value.calculatorFlow,
            ...
            is SearchFieldAction.FocusChanged -> reduceState {
                state.value.copy(searchInFocus = action.focusState.isFocused)
            }
        }
    }

    override fun reduceStateByAction(
        currentState: CustomProductsViewState, action: Action
    ): CustomProductsViewState = when (action) {
    ...
    private fun deleteProduct(dish: DishUiEntity) {
        launch {
            deleteProductUseCase(dish.id).catch { ex ->
                handleError(ex)
    ...
    private fun findProducts(name: String) {
        launch {
            searchCustomProducts(name = name).catch {
                handleError(it)
                ...
                .map { pagingData -> pagingData.map { dishUi -> dishUi.addFirstServing() } }
    ...
    fun setCalculatorFlow(calculatorFlow: CalculatorFlow) {
        reduceState { state.value.copy(calculatorFlow = calculatorFlow) }
    }
}
