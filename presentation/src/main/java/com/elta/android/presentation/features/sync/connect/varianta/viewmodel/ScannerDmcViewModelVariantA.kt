...
class ScannerDmcViewModelVariantA @Inject constructor(
    ...
    override fun reduceStateByAction(
        currentState: ScannerDmcViewStateVariantA,
        action: Action
    ): ScannerDmcViewStateVariantA = run {
        when (action) {
            is ConnectAction.NeedHelp -> reloadSheetContent(ScannerStateVariantA.Help)
            is ConnectAction.CloseHelp -> reloadSheetContent(ScannerStateVariantA.Info)
            else -> currentState
        }
    }
    ...
    private fun setScannerError() {
        if (scannerJob == null || scannerJob?.isCancelled == true) {
            restartCloseTime()
            scannerJob = launch {
                reduceState { state.value.copy(scannerState = ScannerStateVariantA.Error) }
                delay(SCANNER_ERROR_SHOWING_DELAY_MILLIS)
                reduceState { state.value.copy(scannerState = ScannerStateVariantA.Info) }
                delay(SCANNER_ERROR_SHOWING_DELAY_MILLIS)
    ...
    private fun startConnecting(pin: String, name: String) {
        router.navigateTo(Screens.ConnectingScreenVariantA(state.value.isOnBoarding, pin, name))
    }
